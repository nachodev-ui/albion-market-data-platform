...
func BenchmarkNormalizeOrders1000(b *testing.B) { benchmarkNormalizeOrders(b, 1000) }
func BenchmarkNormalizeOrders10000(b *testing.B) { benchmarkNormalizeOrders(b, 10000) }
...
func benchmarkNormalizeOrders(b *testing.B, count int) {
	...
	for iteration := 0; iteration < b.N; iteration++ {
		...
		if len(orders) != count { b.Fatalf("orders=%d want=%d", len(orders), count) }
	}
}
...
func benchmarkService(b *testing.B) *Service {
	...
	if err := os.WriteFile(itemsPath, []byte(items), 0o600); err != nil { b.Fatal(err) }
	...
	if err := os.WriteFile(marketsPath, []byte(markets), 0o600); err != nil { b.Fatal(err) }
...
func benchmarkUpload(count int) domain.MarketOrdersUpload {
	...
	for index := range orders {
		orders[index] = &domain.MarketOrder{
			ID: int64(index + 1), ItemTypeID: fmt.Sprintf("T4_ITEM_%d", index%1000), ItemGroupTypeID: "T4_ITEM",
			LocationID: "3005", QualityLevel: uint8(index%5 + 1), EnchantmentLevel: 0,
			UnitPriceSilver: int64(1000+index) * 10000, Amount: 1, AuctionType: "offer", Expires: "2026-08-02T12:00:00Z",
		}
	}
